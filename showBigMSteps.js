function explainBigMBeginning(numVariables, numEquations) {
    ...
    htmlContent += `Regardless of the function type, we always have to remove <i>M</i>A in the objective function, 
        where A = {all artificial variables we introduced}.<br><br>Here is the initial tableau: <br>`;
...
function showBigMIteration(matrix, numVariables, numEquations, index_i, index_j, count, columnsToRemove, id) {
    ...
    for (let i = 0; i < numEquations; i++) {
        ...
        if (matrix[i][index_j].compare(0) == 1) {
            ...
            (i === index_i ? 
                `<td> <b><sup>${matrix[i][matrix[i].length - 1].toString()}
                </sup>/<sub>${matrix[i][index_j].toString()}</sub> = 
                ${matrix[i][matrix[i].length - 1].div(matrix[i][index_j])} </b> </td> </tr> ` 
                : 
                `<td> <sup>${matrix[i][matrix[i].length - 1].toString()}
                </sup>/<sub>${matrix[i][index_j].toString()}</sub> = 
                ${matrix[i][matrix[i].length - 1].div(matrix[i][index_j])} </td> </tr> `);
    ...
    ${matrix[index_i][index_j].toString()}, at row #${index_i + 1} and column #${realColumn + 1}. <br> <br>`;
