...
function performRowOperations(matrix, numEquations, index_i, index_j) {
    ...
    if (pivot.compare(1) != 0)
        htmlContent += `<b>R<sub>${index_i + 1}</sub> = R<sub>${index_i + 1}</sub> &divide; ${pivot.toFraction()}</b><br>`;
    ...
    for (let i = 0; i < numEquations + 1; i++) {
        ...
        if (ratio instanceof BigMVariable) {
            ...
            if (ratio2.coefficient.compare(0) == 0) {
                htmlContent += `<b>R<sub>${i + 1}</sub> = R<sub>${i + 1}</sub> + `;

                if (ratio2.nonCoefficient.toFraction().search('/') >= 0)
                    htmlContent += `(${ratio2.nonCoefficient.mul(-1).toFraction()})R<sub>${index_i + 1}</sub></b><br>`;

                else if (ratio2.nonCoefficient.compare(-1) != 0)
                    htmlContent += `${ratio2.nonCoefficient.mul(-1).toFraction()}R<sub>${index_i + 1}</sub></b><br>`;

                else
                    htmlContent += `R<sub>${index_i + 1}</sub></b><br>`;
            ...
            // We have a negative M
                ...
                if (negate.toString().search('/') >= 0 && negate.nonCoefficient.compare(0) != 0)
            ...
            if (ratio.compare(1) == 0)
                ...
                htmlContent += `<b>R<sub>${i + 1}</sub> = R<sub>${i + 1}</sub> - `;
                if (ratio.toFraction().search('/') >= 0)
                    htmlContent += `(${ratio.toFraction()})R<sub>${index_i + 1}</sub></b><br>`;
                ...
                if (ratio.toFraction().search('/') >= 0)
                    htmlContent += `(${ratio.mul(-1).toFraction()})R<sub>${index_i + 1}</sub></b><br>`;
...
function displayCoords(matrix, Z, numVariables, numEquations) {
    ...
    for (let i = 0; i < optimals.length; i++) {
        ...
        r.innerHTML += `${optimals[i].toFraction()}`
...
function explainSimplexEnd() {
    ...
    const divSteps = document.getElementById("steps");
    divSteps.innerHTML += '<b>Here is the final tableau:</b><br>';
}
...
function explainSimplexResult(matrix, numVariables, numEquations) {
    ...
    if (matrix[matrix.length - 1][matrix[0].length - 1] instanceof BigMVariable) {
        ...
        if (document.getElementById("functionType").value === "Maximize")
            Z = matrix[matrix.length - 1][matrix[0].length - 1].toString();
        ...
        if (document.getElementById("functionType").value === "Maximize")
            Z = matrix[matrix.length - 1][matrix[0].length - 1].toFraction();

        else {
            Z = matrix[matrix.length - 1][matrix[0].length - 1].mul(-1).toFraction();
            htmlContent += "Our type of function was minimization, so we have to negate Z. <br><br>";
        }
    
    }

    htmlContent += `<b>Therefore, the final solution is Z = ${Z}, at
        (`;
    ...
    for (let i = 0; i < optimals.length; i++) {
        ...
        htmlContent += `${optimals[i].toFraction()}`
